    ...
    <div className='w-full h-full flex flex-col gap-10'>
      {
        ...
        <div className='w-full h-full flex flex-col gap-2'>
          <Typography className={`!font-light ${reverse ? `!text-right` : `!text-left`}`} variant="h2">
            {title}
          </Typography>
          <Typography className={`!font-bold ${reverse ? `!text-right` : `!text-left`}`} variant="h1">
            {name}
          </Typography>
        </div>
      }
        ...
        <div className="relative pb-10 md:px-10 px-5 rounded-2xl flex flex-col gap-2 md:gap-5 h-full md:flex-grow">
          {
            ...
            <div className='w-full h-full flex flex-col gap-2 mt-5'>
              <Typography className={`!font-light !text-center !text-2xl`} variant="h2">
                {title}
              </Typography>
              <Typography className={`!font-bold !text-center !text-3xl`} variant="h1">
                {name}
              </Typography>
            </div>
          }

          <Typography variant="desc" className={`${reverse ? `md:!text-right` : `md:!text-left`}`}>
            {description}
          </Typography>
          <div className={`flex flex-row items-center ${!reverse ? `md:justify-start` : `md:justify-end`} justify-center gap-10`}>
            <Link href={`${facebook}`} className='group flex flex-row gap-2 '>
              ...
              {!isBelowMd && <Typography variant='desc' className='group-hover:text-tamarillo'>Facebook</Typography>}
            </Link>
            <Link href={`${instagram}`} className='group flex flex-row gap-2'>
              ...
              {!isBelowMd && <Typography variant='desc' className='group-hover:text-tamarillo'>Instagram</Typography>}
            </Link>
          </div>
        </div>
      </div>
